...
| Table 4 (FT rerank) + 4.4 stats | `eval_*_xenc-rerank0.3.json`, `h_eval_*_xenc-rerank0.3.json`, `rerank03_rep2.json` (15 reranked runs; the 9 pairwise exceptions in 231/240 are the 3 zero-shot bge runs x 3 baseline repeats, fine-tuned-only is 192/192) vs `b7_*` + baseline aggregates; shortlist recall: `logged_eval_*_xenc-rerank0.3.json` (3 byte-identical logs = one measurement; the harness logged post-normalization query strings, so the join to benchmark records uses the checked, order-preserving 308-row mapping in `../results/shortlist_join_map.csv`, validated and recomputed by `verify_claims.py`: recall@3 .864->.886, recall@1 .740->.708) |
...
| Residual-failure taxonomy / 38 failures | 4 repeats used for the residual-failure taxonomy (identified as the set whose majority-failure set reproduces the label-audit record): `b7_1711.json`, `b7_1720.json`, `b7_1723.json`, `b7_1726.json` (majority = fail in >=2 of 4 -> 38; ever-failing 43). All 7 released b7 repeats give 35 majority / 46 ever-failing. |
| 5 label audit | `../adjudication/adjudication_record_FINAL.csv` (legacy directory/file names; sole-author label audit, not independent adjudication) |
